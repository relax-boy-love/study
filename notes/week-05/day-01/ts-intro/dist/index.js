...
(function (CourseStatus) {
    CourseStatus["Learning"] = "learning";
    CourseStatus["Completed"] = "completed";
})(CourseStatus || (CourseStatus = {}));
function createCourseSummary(courseName, scores, status) {
...
const summary = createCourseSummary("TypeScript", [80, 90, 100], CourseStatus.Learning);
const matchedTopic = findTopic(["类型注解", "类型推导", "严格模式"], "泛型");
console.log(summary);
console.log(matchedTopic);
// 1. createCourseSummary 的三个参数分别是什么类型？ string number[]  string
// 2. 它返回的元组类型是什么？  [string, number, string]
// 3. findTopic 的三个空白分别填写什么？  string[]  string string
// 4. summary 的总分是多少？270
// 5. matchedTopic 最终是什么？  未找到
// 6. 为什么 findTopic 的返回类型可以只写 string，不需要写 string | undefined？  因为if已经类型缩小到了string
